...
import Sistema.validadorDecpf.ValidaCpf;
...
public class BancoRepository {
    public static void InsertCliente(String name, String cpf) {
        ...
        ClienteCadastrado cliente = ClienteCadastrado.builder().build();
        System.out.println(cliente.retornaCpfCliente());
        ...
        try(Connection conn = ConnectionFactory.connectionFactory();
            ...
            if (rs.next()) {
                ClienteCadastrado clienteCadastrado = ClienteCadastrado.builder()
                        ...
                        .cpf("cpf")
                        .build();
                clienteCadastrado.adicionaCpfCliente(clienteCadastrado.getCpf());
    ...
    public static void DeleteClientePutId(int id) {
        try(Connection conn = ConnectionFactory.connectionFactory();
            PreparedStatement ps = deleteCliente(conn, id)) {
            int linhasAfetadas = ps.executeUpdate();
            if (linhasAfetadas == 0) throw new IllegalArgumentException("esse id nao existe");
        } catch (SQLException e) {
            throw new RuntimeException(e);
        }
    }
    ...
    private static PreparedStatement deleteCliente(Connection conn, int id) throws SQLException {
        String sql = "DELETE FROM `Banco_do_serasa`.`Cliente` WHERE id=(?);";
        PreparedStatement ps = conn.prepareStatement(sql);
        ps.setInt(1, id);
        return ps;
    }
    private static boolean validaCpf(String cpf) {
        boolean cpfIsReal = ValidaCpf.validaCpf(cpf);
        return cpfIsReal;
    }
}
